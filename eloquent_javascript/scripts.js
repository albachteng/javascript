...
// below are implementations of prebuilt functions, commented out; 
// pre-built array functions are called as a method on strings
// and therefore use dot notation instead of a regular function call;
...
//     let current = start;
//     for (let element of arr) {
//         current = combine(current, element); // combines each element with the next
//     } // and then repeats with the newly combined element until there are no more;
//     return current;
// }

// standard array method reduce also allows you to ignore the start argument
// if and only if there is at least one item in the array 

// console.log(reduce([1, 2, 3], (x, y) => x+y, 0));
...
let oldest = SCRIPTS.reduce((a, b) => {
    return a.year < b.year ? a : b;
});
console.log(oldest);

// find avg year of origin for living scripts and then for dead scripts

function average(array) {
    return array.reduce((a, b) => a + b) / array.length;
}

let avgLiving = Math.round(average(SCRIPTS.filter(a => a.living).map(s => s.year)));
console.log(avgLiving);
let avgDead = Math.round(average(SCRIPTS.filter(a=>!a.living).map(s=>s.year)));
console.log(avgDead);

function characterScript(code) { // identifies a script from a character code
...
console.log(characterScript(121)); // expect Latin object

// some => returns true if at least one item in the array...
// ...returns true for the test function

const horseshoe = "🐎👟";
console.log(horseshoe.charCodeAt(0)); // 55357 charcode of HALF of the horse
console.log(horseshoe.length); // expect 4, even though only 2 emoji
// this is bc emoji are mostly two-code characters
console.log(horseshoe.codePointAt(0)); // expect actual code for horse
// codePointAt is a safer method for dealing with non-english characters/emoji
// to avoid issues looping over strings with potentially more than one code per character
// use a for/of loop on the string, which accounts for this

for (let char of horseshoe) {
    console.log(char); // expect 🐎 + 👟 
}

function countBy(items, groupName) {
    let counts = [];
    // stores objects of format {name: name, count: count};
    for (let item of items) {
        let name = groupName(item);
        // function generates the groupName for a given item
        let known = counts.findIndex(c => c.name == name);
        // findIndex returns the first value for which the callback function is true
        // if none are true, it returns -1;
        if (known == -1) {
            counts.push({name, count: 1});
            // if there are no objects yet with that name, create it and initialize count to 1
        } else {
            counts[known].count++;
            // if there IS an object with that name, increment the count of that object instead
        }
    }
    return counts; 
}

console.log(countBy([1, 2, 3, 4, 5], n => n > 2)); // expect an array of two objects
// => [{name: false, count: 2}, {name: true, count: 3}]

function textScripts(text) {
    let scripts = countBy(text, char => { // countBy checks each character
        let script = characterScript(char.codePointAt(0));
        return script ? script.name : "none"; // if not recognized, unknown script
    }).filter(({name}) => name != "none"); // filter out the unknown scripts

    let total = scripts.reduce((n, {count}) => n + count, 0);
    if (total === 0) return "no scripts found"; // iterate over the scripts list
    //using reduce to find the total number of scripts that were found in the text

    return scripts.map(({name, count}) => {
        return `${Math.round(count * 100 / total)}% ${name}`;
    }).join(', '); // calculate percentages for each name and count
    // join those name/percentage pairs with commas into a single string
}

console.log(textScripts('英国的狗说"woof", 俄罗斯的狗说"тяв"')); // 61% Han, 22% Latin, 17% Cyrillic

function flatten(array) {
    let flattened = array.reduce((accumulator, subarray) => {
        return accumulator.concat(subarray);
    })
    return flattened;
}

console.log(flatten([[1, 2, 3], [4], [5, 6],])); // expect [1, 2, 3, 4, 5, 6];
...
// an alternate implementation of every using the some() method;

//   function every(array, test) {
//     return !array.some(x => !test(x));
//   }

  console.log(every([1, 2, 3], n => n > 0)); // expect true;
  ...
  function dominantDirection(text) {
    ...
    let name = scripts.length == 1 ? scripts[0].name : scripts.reduce((a, b) => {
      return (a.count < b.count ? b.name : a.name);
    });
    
    console.log(name);
      
    return SCRIPTS[SCRIPTS.findIndex((script) => script.name == name)].direction;
